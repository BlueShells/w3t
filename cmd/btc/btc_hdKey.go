...
func init() {
	...
	hdkeyCmd.Flags().StringP("pass", "p", "", "protect bip39 mnemonic with a passphrase")
	...
	hdkeyCmd.Flags().StringP("wif", "w", "", "decode the private key from wif, then generate the bitcoin address.")
	hdkeyCmd.Flags().StringP("wif-network", "n", "testnet", "Network corresponding to the WIF.")

	viper.BindPFlag("mnemonic", hdkeyCmd.Flags().Lookup("mnemonic"))
}
...
func run(cmd *cobra.Command, args []string) {
	...
	wifNetwork, _ := cmd.Flags().GetString("wif-network")
